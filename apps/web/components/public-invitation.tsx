...
function GuestbookPreviewSection({ entries, albumToken }: { entries: NonNullable<PublicInvitationData["guestbookEntries"]>; albumToken?: string }) {
  ...
  return <section className="inv4-section inv-social-guestbook" aria-labelledby="guestbook-preview-title">
    ...
    <div className="inv-social-wish-grid">{entries.slice(0, 6).map((entry) => <blockquote key={entry.id}><span aria-hidden="true">“</span><p>{entry.message}</p><footer><span className="memory-wish-avatar" aria-hidden="true">{entry.authorName.trim().charAt(0).toUpperCase() || "N"}</span><strong>{entry.authorName}</strong></footer></blockquote>)}</div>
