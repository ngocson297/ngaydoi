...
export default function CheckinPage() {
  ...
  if (loading && !data) return <main id="main-content" tabIndex={-1} className="checkin-shell checkin-page"><PageSkeleton cards={2} /></main>;
  if (stationError || !data) return <main id="main-content" tabIndex={-1} className="checkin-shell checkin-page"><section className="checkin-card"><ErrorState title="Không thể mở trạm check-in" description={stationError?.message ?? "Trạm check-in không còn khả dụng."} requestId={stationError?.requestId} onRetry={() => void load()} homeHref="/" homeLabel="Về trang chủ" compact /></section></main>;
  return <main id="main-content" tabIndex={-1} className="checkin-shell checkin-page">
    ...
    <div className="checkin-layout">
      <section className="checkin-card checkin-main-card"><div className="checkin-tabs"><strong>Quét QR hoặc tìm khách</strong><span>Luôn có thể nhập tên nếu camera không hoạt động</span></div>
        ...
        <div className="station-search-results">{guests.map((guest) => <button key={guest.id} className={selected?.id === guest.id ? "selected" : ""} onClick={() => choose(guest)}><span className="guest-avatar">{guest.fullName.charAt(0)}</span><div><strong>{guest.fullName}</strong><small>{guest.groupName || "Chưa phân nhóm"}{guest.assignment ? ` · ${guest.assignment.table.name}` : " · Chưa có bàn"}</small></div>{guest.checkin && !guest.checkin.checkedOutAt ? <em>Đã check-in</em> : <i>Chưa check-in</i>}</button>)}</div>
        {selected && <div className="checkin-confirm"><div><span>Khách được chọn</span><h2>{selected.fullName}</h2><p>{selected.assignment ? `Bàn: ${selected.assignment.table.name}${selected.assignment.table.zone ? ` · ${selected.assignment.table.zone}` : ""}` : "Khách chưa được phân bàn"}</p></div>{selected.checkin && !selected.checkin.checkedOutAt ? <div className="alert alert-success">Khách đã check-in lúc {new Date(selected.checkin.checkedInAt).toLocaleTimeString("vi-VN")}</div> : <><div className="people-stepper"><label>Người lớn<input type="number" min={0} max={20} value={counts.adults} onChange={(e) => setCounts({ ...counts, adults: Number(e.target.value) })} /></label><label>Trẻ em<input type="number" min={0} max={20} value={counts.children} onChange={(e) => setCounts({ ...counts, children: Number(e.target.value) })} /></label></div><button className="btn btn-primary full-button" disabled={busy || counts.adults + counts.children < 1} onClick={() => void checkIn({ guestId: selected.id })}>{busy ? "Đang ghi nhận..." : `Check-in ${counts.adults + counts.children} người`}</button></>}</div>}
      </section>
