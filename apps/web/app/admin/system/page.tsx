...
function SystemContent() {
  ...
  return <AppShell active="system">
    <div className="page-heading system-heading"><div><p className="eyebrow">PRODUCTION READINESS</p><h1>Trung tâm vận hành hệ thống</h1><p>Theo dõi database, storage, email, webhook và cấu hình triển khai trong một màn hình.</p></div><a className="btn btn-secondary" href="/status" target="_blank" rel="noreferrer">Mở status page ↗</a></div>
    <Tabs<Tab> id="system-tabs" label="Khu vực vận hành hệ thống" className="studio-tabs system-tabs" value={tab} onChange={setTab} items={[{ value: "overview", label: "Tổng quan" }, { value: "email", label: "Email Outbox" }, { value: "webhooks", label: "Webhooks" }]} />
    ...
    {tab === "email" && <section className="panel operations-panel" {...tabPanelProps("system-tabs", "email")}><div className="panel-head"><div><h2>Email Outbox</h2><p className="muted-small">Email được lưu trước, gửi nền và tự retry khi provider gián đoạn.</p></div><button disabled={busy} className="btn btn-primary" onClick={() => void run("/admin/system/emails/process", "Đã xử lý hàng đợi email.")}>Xử lý ngay</button></div><div className="mini-kpis"><span>Provider <b>{overview?.mail.provider}</b></span><span>Chờ gửi <b>{overview?.mail.metrics.pending ?? 0}</b></span><span>Đã gửi <b>{overview?.mail.metrics.delivered ?? 0}</b></span><span>Dead letter <b>{overview?.mail.metrics.deadLetter ?? 0}</b></span></div><div className="ops-table">{overview?.mail.recent.map((item) => <article key={item.id}><div><strong>{item.subject}</strong><span>{item.recipient} · {new Date(item.createdAt).toLocaleString("vi-VN")}</span>{item.lastError ? <small>{item.lastError}</small> : null}</div><em className={`delivery-state ${item.status.toLowerCase()}`}>{item.status}</em>{["FAILED", "DEAD_LETTER"].includes(item.status) ? <button disabled={busy} onClick={() => void run(`/admin/system/emails/${item.id}/retry`, "Email đã được đưa lại vào hàng đợi.")}>Retry</button> : null}</article>)}</div></section>}

    {tab === "webhooks" && <div className="webhook-layout" {...tabPanelProps("system-tabs", "webhooks")}><section className="panel webhook-create"><h2>Thêm webhook</h2><p>Ngày Đôi ký mỗi request bằng HMAC SHA-256. Secret chỉ hiển thị một lần sau khi tạo.</p><form onSubmit={(event) => void createWebhook(event)}><label>Tên endpoint<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="ERP / CRM / Automation" /></label><label>HTTPS URL<input required type="url" value={form.url} onChange={(event) => setForm({ ...form, url: event.target.value })} placeholder="https://example.com/webhooks/ngaydoi" /></label><label>Sự kiện, cách nhau bằng dấu phẩy<input required value={form.events} onChange={(event) => setForm({ ...form, events: event.target.value })} /></label><button disabled={busy} className="btn btn-primary full">Tạo webhook</button></form>{createdSecret ? <div className="secret-box"><strong>Signing secret — lưu ngay</strong><code>{createdSecret}</code><button onClick={() => void navigator.clipboard.writeText(createdSecret)}>Sao chép</button></div> : null}</section>
      <section className="panel webhook-list"><div className="panel-head"><div><h2>Endpoints</h2><p className="muted-small">Retry tối đa 6 lần với exponential backoff.</p></div><button disabled={busy} className="btn btn-secondary" onClick={() => void run("/admin/system/webhooks/process", "Đã xử lý hàng đợi webhook.")}>Xử lý hàng đợi</button></div>{endpoints.length ? endpoints.map((endpoint) => <article key={endpoint.id}><div className="webhook-main"><span className={`service-dot ${endpoint.active ? "operational" : "outage"}`} /><div><strong>{endpoint.name}</strong><span>{endpoint.url}</span><small>{endpoint.events.join(" · ")} · {endpoint.deliveryCount ?? 0} deliveries</small></div></div><aside><em>{endpoint.failureCount ? `${endpoint.failureCount} lỗi` : "Ổn định"}</em><button disabled={busy} onClick={() => void run(`/admin/system/webhooks/${endpoint.id}/test`, "Đã gửi webhook test.")}>Test</button></aside></article>) : <div className="admin-empty"><strong>Chưa có webhook</strong><p>Thêm endpoint để đồng bộ RSVP và thanh toán sang hệ thống khác.</p></div>}</section>
      <section className="panel webhook-deliveries"><h2>50 deliveries gần nhất</h2><div className="ops-table">{deliveries.map((item) => <article key={item.id}><div><strong>{item.eventType}</strong><span>{item.endpoint.name} · {new Date(item.createdAt).toLocaleString("vi-VN")}</span>{item.lastError ? <small>{item.lastError}</small> : null}</div><em className={`delivery-state ${item.status.toLowerCase()}`}>{item.responseStatus ?? item.status}</em>{["FAILED", "DEAD_LETTER"].includes(item.status) ? <button disabled={busy} onClick={() => void run(`/admin/system/webhook-deliveries/${item.id}/retry`, "Webhook đã được đưa lại vào hàng đợi.")}>Retry</button> : null}</article>)}</div></section></div>}
  </AppShell>;
}
