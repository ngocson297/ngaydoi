...
function Content() {
  ...
  if (!allowed) return <AppShell active="growthAdmin"><div className="page-state-panel"><PermissionState description="Tăng trưởng & Chăm sóc khách hàng chỉ dành cho Admin và Staff." /></div></AppShell>;
  ...
  return <AppShell active="growthAdmin"><div className={styles.consolePage}>
    <div className={`${styles.consoleHeader} page-heading`}><div><p className="eyebrow">GROWTH OPERATIONS</p><h1>Tăng trưởng & Chăm sóc khách hàng</h1><p>Theo dõi phễu chuyển đổi, hỗ trợ khách hàng, tên miền và referral trong một không gian vận hành thống nhất.</p></div></div>
    {error ? <InlineErrorState description={error.message} requestId={error.requestId} onRetry={() => void load()} /> : null}
    <Tabs<GrowthTab> id="growth-tabs" label="Khu vực Growth Center" className={`${styles.consoleTabs} studio-tabs`} value={tab} onChange={setTab} items={[{ value: "funnel", label: "Phễu chuyển đổi" }, { value: "support", label: "Hỗ trợ" }, { value: "domains", label: "Tên miền" }, { value: "referrals", label: "Giới thiệu" }]} />
    {tab === "funnel" ? <section className="panel" {...tabPanelProps("growth-tabs", "funnel")}><h2>Phễu chuyển đổi · 30 ngày</h2><div className="funnel-grid">{funnel.map(([name, value], index) => <article key={name}><span>{index + 1}</span><div><small>{name}</small><strong>{value}</strong></div></article>)}</div></section> : null}
    {tab === "support" ? <section className="panel table-scroll state-section" {...tabPanelProps("growth-tabs", "support")}><h2>Hộp thư hỗ trợ</h2>{loading ? <TableSkeleton rows={5} columns={3} /> : data?.tickets.length ? data.tickets.map((ticket) => <article className="ops-row" key={ticket.id}><div><strong>{ticket.subject}</strong><p>{ticket.email} · {ticket.category} · {ticket.priority}</p></div><select className={styles.compactSelect} value={ticket.status} onChange={(event) => void patch(`/admin/growth/tickets/${ticket.id}`, { status: event.target.value })}><option>OPEN</option><option>IN_PROGRESS</option><option>WAITING_CUSTOMER</option><option>RESOLVED</option><option>CLOSED</option></select></article>) : <EmptyState compact icon="✓" title="Support inbox đang trống" description="Không có yêu cầu hỗ trợ cần xử lý." />}</section> : null}
    {tab === "domains" ? <section className="panel state-section" {...tabPanelProps("growth-tabs", "domains")}><h2>Hàng đợi tên miền</h2>{loading ? <TableSkeleton rows={4} columns={3} /> : data?.domains.length ? data.domains.map((domain) => <article className="ops-row" key={domain.id}><div><strong>{domain.hostname}</strong><p>{domain.weddingId} · {domain.status}</p></div><select className={styles.compactSelect} value={domain.status} onChange={(event) => void patch(`/admin/growth/domains/${domain.id}`, { status: event.target.value })}><option>PENDING_DNS</option><option>VERIFYING</option><option>VERIFIED</option><option>ACTIVE</option><option>FAILED</option><option>DISABLED</option></select></article>) : <EmptyState compact icon="◎" title="Không có tên miền chờ xử lý" description="Yêu cầu tên miền mới sẽ xuất hiện tại đây." />}</section> : null}
    {tab === "referrals" ? <section className="panel state-section" {...tabPanelProps("growth-tabs", "referrals")}><h2>Hiệu quả giới thiệu</h2>{loading ? <TableSkeleton rows={4} columns={3} /> : data?.referrals.length ? data.referrals.map((referral) => <article className="ops-row" key={referral.id}><div><strong>{referral.code}</strong><p>{referral.label || "Không nhãn"}</p></div><div className="mini-metrics"><span>{referral.visitCount} visit</span><span>{referral.signupCount} signup</span><span>{referral.conversionCount} paid</span></div></article>) : <EmptyState compact icon="↗" title="Chưa có dữ liệu referral" description="Hiệu suất mã giới thiệu sẽ xuất hiện sau khi người dùng bắt đầu chia sẻ." />}</section> : null}
  </div></AppShell>;
}
