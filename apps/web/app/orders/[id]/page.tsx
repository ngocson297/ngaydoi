...
function OrderContent() {
  ...
  return <AppShell active="billing">
    <div className="order-detail-head"><div><a className="back-link" href="/billing">← Quay lại đơn hàng</a><div className="eyebrow">{order.orderNumber}</div><h1>{order.plan.name} · {order.wedding.title}</h1><p>Tạo ngày {new Date(order.createdAt).toLocaleString("vi-VN")}</p></div><div className="order-total-box" aria-label="Tổng thanh toán"><div className="order-total-meta"><span>Tổng thanh toán</span><em className={`commerce-status ${order.paymentStatus.toLowerCase()}`}>{paymentStatusLabels[order.paymentStatus] ?? order.paymentStatus}</em></div><strong>{formatMoney(order.totalAmount)}</strong><small>Giá trị cuối cùng của đơn hàng</small></div></div>
    ...
    <div className="order-detail-grid">
      <div className="order-detail-main">
        ...
        {completed && <section className="receipt-card receipt-document" aria-labelledby="receipt-title">
          <header className="receipt-header">
            <div className="receipt-brand"><span>Ngày</span> <strong>Đôi</strong><small>Wedding experience platform</small></div>
            <div className="receipt-paid-mark" aria-label="Đã thanh toán"><span aria-hidden="true">✓</span><strong>ĐÃ THANH TOÁN</strong></div>
          </header>
          <div className="receipt-title-block"><div><small>BIÊN NHẬN DỊCH VỤ ĐIỆN TỬ</small><h2 id="receipt-title">Biên nhận thanh toán</h2><p>Chứng từ xác nhận quyền lợi dịch vụ đã được kích hoạt thành công.</p></div><div className="receipt-number"><span>Số biên nhận</span><strong>{order.orderNumber}</strong></div></div>
          <div className="receipt-context-grid"><div><span>Khách hàng / Wedding</span><strong>{order.wedding.title}</strong><small>Gói dịch vụ: {order.plan.name}</small></div><div><span>Ngày xác nhận</span><strong>{payment?.confirmedAt ? new Date(payment.confirmedAt).toLocaleDateString("vi-VN") : "Đã xác nhận"}</strong><small>{payment?.confirmedAt ? new Date(payment.confirmedAt).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" }) : "Hệ thống Ngày Đôi"}</small></div><div><span>Phương thức</span><strong>{payment?.method === "SANDBOX" ? "Thanh toán thử nghiệm" : "Chuyển khoản ngân hàng"}</strong><small>{payment?.reference || "Đã được đối soát"}</small></div></div>
          <div className="receipt-table" role="table" aria-label="Chi tiết biên nhận">
            <div className="receipt-table-head" role="row"><span role="columnheader">Hạng mục</span><span role="columnheader">Phân loại</span><span role="columnheader">Số tiền</span></div>
            {order.items.map((item) => <div className="receipt-table-row" role="row" key={item.id}><span role="cell"><strong>{item.name}</strong><small>Số lượng {item.quantity}</small></span><span role="cell">{item.itemType === "PLAN" ? "Gói dịch vụ" : "Tiện ích bổ sung"}</span><strong role="cell">{formatMoney(item.totalAmount)}</strong></div>)}
          </div>
          <div className="receipt-summary">
            <div><span>Tạm tính</span><strong>{formatMoney(order.subtotalAmount)}</strong></div>
            {order.discountAmount > 0 ? <div className="discount"><span>Ưu đãi {order.coupon?.code}</span><strong>-{formatMoney(order.discountAmount)}</strong></div> : null}
            <div className="receipt-grand-total"><span>Tổng thanh toán</span><strong>{formatMoney(order.totalAmount)}</strong></div>
          </div>
          <footer className="receipt-footer"><p>Biên nhận được tạo tự động bởi hệ thống Ngày Đôi. Vui lòng lưu mã đơn <strong>{order.orderNumber}</strong> khi cần hỗ trợ.</p><div className="receipt-actions no-print"><a className="btn btn-primary" href={`/weddings/${order.wedding.id}`}>Mở wedding</a><button className="btn btn-secondary" type="button" onClick={() => window.print()}>In biên nhận</button></div></footer>
        </section>}
      </div>
