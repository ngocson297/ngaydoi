...
<script>
  ...
  if (!data) return <AppShell active="eventOps" weddingId={weddingId}><ErrorState title="Không thể mở khu vực vận hành" description={loadError?.message ?? "Dữ liệu vận hành chưa sẵn sàng."} requestId={loadError?.requestId} onRetry={() => void load()} homeHref={`/weddings/${weddingId}`} homeLabel="Về không gian đám cưới" /></AppShell>;

  return <AppShell active="eventOps" weddingId={weddingId}>
    <div className="event-operations-page">
    <a className="back-link" href={`/weddings/${weddingId}`}>← Về không gian đám cưới</a>
    ...
    {tab === "guests" && <section className="panel" {...tabPanelProps("event-operations-tabs", "guests")}><div className="panel-head ops-panel-head"><div><h2>Phân khách vào bàn</h2><p className="muted-small">Hệ thống mặc định lấy số ghế từ RSVP. Bạn có thể đổi bàn bất kỳ lúc nào.</p></div>{canEdit && <button className="btn btn-secondary" disabled={busy || !data.tables.length || !unassigned.length} onClick={() => void autoAssign()}>Tự động phân bàn</button>}</div><div className="ops-filter"><input aria-label="Tìm khách để phân bàn" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Tìm tên, nhóm, điện thoại hoặc email..." /><span>{filteredGuests.length} khách</span></div><div className="guest-assignment-list">{filteredGuests.map((guest) => <article key={guest.id}><div className="guest-avatar">{guest.fullName.charAt(0).toUpperCase()}</div><div className="guest-assignment-info"><strong>{guest.fullName}</strong><span>{guest.groupName || "Chưa phân nhóm"} · {guest.assignment ? `Bàn ${data.tables.find((table) => table.assignments.some((item) => item.id === guest.assignment?.id))?.name ?? "đã xếp"}` : "Chưa xếp bàn"} · {guest.invitation?.rsvp?.status === "ATTENDING" ? `${guest.invitation.rsvp.adultCount + guest.invitation.rsvp.childCount} người tham dự` : "Chưa xác nhận tham dự"}</span></div><div className="guest-table-control">{canEdit ? <><span className="assignment-action-label">{guest.assignment ? "Chuyển bàn" : "Xếp bàn"}</span><select aria-label={`${guest.assignment ? "Chuyển bàn cho" : "Xếp bàn cho"} ${guest.fullName}`} value={guest.assignment ? data.tables.find((table) => table.assignments.some((item) => item.id === guest.assignment?.id))?.id ?? "" : ""} onChange={(e) => e.target.value ? void assign(guest, e.target.value) : void unassign(guest)}>{guest.assignment ? <option value="">Bỏ xếp bàn</option> : <option value="">Chọn bàn</option>}{data.tables.map((table) => <option key={table.id} value={table.id}>{table.name} · {occupiedSeats(table)}/{table.capacity}</option>)}</select></> : <span>{guest.assignment ? "Đã xếp bàn" : "Chưa xếp bàn"}</span>}</div></article>)}</div></section>}
