      ...
      {tab === "events" && (
        <div className="workspace-layout events-layout" {...tabPanelProps("workspace-tabs", "events")}>
          ...
          {canEdit && <form id="event-form" className="panel event-form" onSubmit={(event) => void saveEvent(event)}><div className="panel-head"><div><h2>{editingEventId ? "Sửa sự kiện" : "Thêm sự kiện"}</h2><p className="muted-small">Thời gian được lưu theo ISO và múi giờ đã chọn.</p></div>{editingEventId && <button type="button" className="text-button" onClick={resetEventForm}>Hủy sửa</button>}</div><div className="form-grid two event-form-grid">
            ...
            <label className="full">Bắt đầu<input required type="datetime-local" value={eventDraft.startsAt} onChange={(e) => setEventDraft({ ...eventDraft, startsAt: e.target.value })} /></label>
            <label className="full">Kết thúc<input type="datetime-local" value={eventDraft.endsAt ?? ""} onChange={(e) => setEventDraft({ ...eventDraft, endsAt: e.target.value || null })} /></label>
