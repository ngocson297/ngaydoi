...
const publishReviewLabels: Record<WeddingDetail["publishReviewStatus"], string> = {
    ...
    <AppShell active="weddings" weddingId={weddingId}>
      ...
      <div className="workspace-header">
        ...
        <div className="workspace-header-actions">
          <a className="btn btn-primary workspace-main-cta" href={`/weddings/${weddingId}/invitation`}>{canEdit ? "Chỉnh sửa thiệp" : "Xem thiết kế"}<span aria-hidden="true">→</span></a>
          ...
          {isOwner && <a className="btn btn-secondary workspace-plan-cta" href={`/pricing?weddingId=${weddingId}`}>{wedding.activePlan ? "Quản lý gói" : "Chọn gói"}</a>}
        </div>
      </div>
      ...
      {tab === "details" && (
        <form className="panel workspace-form" {...tabPanelProps("workspace-tabs", "details")} onSubmit={(event) => void saveDetails(event)}>
          ...
          <div className="form-section story-section"><h3>Câu chuyện</h3><label>Giới thiệu ngắn<textarea disabled={!canEdit} rows={5} maxLength={3000} value={details.story} onChange={(e) => setDetails({ ...details, story: e.target.value })} placeholder="Từ một cuộc gặp tình cờ..." /></label></div>
      ...
      {tab === "events" && (
        <div className="workspace-layout events-layout" {...tabPanelProps("workspace-tabs", "events")}>
          <section className="event-list">
            {wedding.events.length === 0 ? <div className="empty-panel timeline-empty"><div className="empty-icon">◷</div><h3>Chưa có sự kiện</h3><p>Thêm lễ cưới, tiệc cưới hoặc các mốc quan trọng.</p></div> : wedding.events.map((item) => <article className="event-manage-card timeline-event-card" key={item.id}><div className="event-index">{String(item.sortOrder + 1).padStart(2, "0")}</div><div><div className="event-tags"><span>{sideLabels[item.side]}</span><span>{eventTypeLabels[item.type]}</span></div><h3>{item.title}</h3><p><strong>{formatDate(item.startsAt, true)}</strong><br />{item.venueName}<br />{item.address}</p>{item.dressCode && <small>Trang phục: {item.dressCode}</small>}</div>{canEdit && <div className="row-actions"><button onClick={() => beginEditEvent(item)}>Sửa</button><button className="danger-link" disabled={busy} onClick={() => setDeleteEventTarget(item.id)}>Xóa</button></div>}</article>)}
          </section>
          {canEdit && <form id="event-form" className="panel event-form" onSubmit={(event) => void saveEvent(event)}><div className="panel-head"><div><h2>{editingEventId ? "Sửa sự kiện" : "Thêm sự kiện"}</h2><p className="muted-small">Thời gian được lưu theo ISO và múi giờ đã chọn.</p></div>{editingEventId && <button type="button" className="text-button" onClick={resetEventForm}>Hủy sửa</button>}</div><div className="form-grid two event-form-grid">
