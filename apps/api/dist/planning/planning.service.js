...
import { PlanningTaskCategory, PlanningTaskPriority, PlanningTaskSource, PlanningTaskStatus, } from "../generated/prisma/client.js";
import { MailService } from "../operations/mail.service.js";
import { PrismaService } from "../prisma/prisma.service.js";
const DAY = 86_400_000;
const INACTIVE_STATUSES = [PlanningTaskStatus.DONE, PlanningTaskStatus.CANCELED];
...
const dateOrNull = (value) => {
    ...
    if (Number.isNaN(parsed.getTime()))
        throw new BadRequestException("Ngày hoàn thành không hợp lệ");
    return parsed;
};
const enumValue = (value, allowed, fallback) => {
    const normalized = trim(value, 40);
...
let PlanningService = PlanningService_1 = class PlanningService {
    prisma;
    mail;
    logger = new Logger(PlanningService_1.name);
    timer;
    ...
    async access(weddingId, user, edit = false) {
        ...
        if (!access)
            throw new NotFoundException("Không tìm thấy đám cưới");
        if (edit && access === "VIEW")
            throw new ForbiddenException("Bạn chỉ có quyền xem kế hoạch cưới");
        return { access, wedding };
    }
    async overview(weddingId, user) {
    ...
    async bootstrap(weddingId, user) {
        const { wedding } = await this.access(weddingId, user, true);
        if (!wedding.mainDate)
            throw new BadRequestException("Hãy cập nhật ngày cưới trước khi tạo timeline tự động");
        ...
        if (rows.length)
            await this.prisma.planningTask.createMany({ data: rows });
        return { created: rows.length, skipped: TEMPLATE_TASKS.length - rows.length };
    }
    async create(weddingId, body, user) {
        await this.access(weddingId, user, true);
        const title = trim(body.title, 120);
        if (title.length < 3)
            throw new BadRequestException("Tên công việc cần ít nhất 3 ký tự");
    ...
    async update(weddingId, taskId, body, user) {
        await this.access(weddingId, user, true);
    ...
    async remove(weddingId, taskId, user) {
        await this.access(weddingId, user, true);
    ...
    async processReminders() {
        ...
        for (const task of candidates) {
            if (!task.dueAt || task.dueAt.getTime() > now.getTime() + task.reminderDaysBefore * DAY)
                continue;
...
PlanningService = PlanningService_1 = __decorate([
    Injectable(),
