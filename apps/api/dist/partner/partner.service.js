...
const slugify = (value) => value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 48);
let PartnerService = class PartnerService {
    ...
    async membership(userId) {
        ...
        if (!member)
            throw new ForbiddenException("Tài khoản chưa thuộc một đối tác.");
    ...
    async overview(user) {
        ...
        if (!membership)
            return { partner: null, clients: [], commissions: [], payouts: [], metrics: { clients: 0, weddings: 0, pending: 0, payable: 0, paid: 0 } };
    ...
    async apply(user, body) {
        ...
        if (existing)
            throw new BadRequestException("Tài khoản đã thuộc một đối tác.");
        const name = String(body.name ?? "").trim();
        if (name.length < 3)
            throw new BadRequestException("Tên đối tác cần ít nhất 3 ký tự.");
        const base = slugify(name) || `partner-${Date.now()}`;
        let slug = base;
        let suffix = 1;
        while (await this.prisma.partnerOrganization.findUnique({ where: { slug } }))
            slug = `${base}-${suffix++}`;
    ...
    async updateProfile(user, body) {
        ...
        if (!["OWNER", "MANAGER"].includes(member.role))
            throw new ForbiddenException("Bạn không có quyền cập nhật thương hiệu.");
        const primaryColor = body.primaryColor ? String(body.primaryColor) : undefined;
        if (primaryColor && !/^#[0-9A-F]{6}$/i.test(primaryColor))
            throw new BadRequestException("Màu thương hiệu không hợp lệ.");
    ...
    async addClient(user, body) {
        const { partner } = await this.membership(user.id);
        if (partner.status !== "ACTIVE")
            throw new ForbiddenException("Đối tác cần được duyệt trước khi thêm khách hàng.");
        const email = String(body.email ?? "").trim().toLowerCase();
        const customer = await this.prisma.user.findUnique({ where: { email } });
        if (!customer)
            throw new NotFoundException("Khách hàng cần có tài khoản Ngày Đôi trước.");
        ...
        if (weddingId) {
            ...
            if (!wedding)
                throw new BadRequestException("Wedding không thuộc khách hàng này.");
        }
        const existing = await this.prisma.partnerClient.findFirst({ where: { partnerId: partner.id, customerId: customer.id, weddingId } });
        if (existing)
            return this.prisma.partnerClient.update({ where: { id: existing.id }, data: { notes: body.notes ? String(body.notes).slice(0, 500) : null, externalRef: body.externalRef ? String(body.externalRef).slice(0, 80) : null } });
    ...
    async requestPayout(user, body) {
        const { member, partner } = await this.membership(user.id);
        if (!["OWNER", "MANAGER"].includes(member.role))
            throw new ForbiddenException("Bạn không có quyền yêu cầu thanh toán.");
        if (partner.status !== "ACTIVE")
            throw new ForbiddenException("Đối tác chưa hoạt động.");
        ...
        if (amount < partner.minimumPayout || amount > available)
            throw new BadRequestException(`Số tiền cần từ ${partner.minimumPayout.toLocaleString("vi-VN")}đ đến số dư khả dụng.`);
        const bankName = String(body.bankName ?? "").trim();
        const accountName = String(body.accountName ?? "").trim();
        const accountNumber = String(body.accountNumber ?? "").replace(/\s/g, "");
        if (!bankName || !accountName || accountNumber.length < 6)
            throw new BadRequestException("Thông tin nhận tiền chưa đầy đủ.");
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            for (const commission of commissions) {
                if (remaining <= 0)
                    break;
                await tx.partnerCommission.update({ where: { id: commission.id }, data: { payoutId: payout.id, status: "PAYABLE" } });
    ...
    async reviewPartner(user, id, body) {
    ...
    async reviewPayout(user, id, body) {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (status === "PAID")
                await tx.partnerCommission.updateMany({ where: { payoutId: id }, data: { status: "PAID", paidAt: new Date() } });
            if (["REJECTED", "CANCELED"].includes(status))
                await tx.partnerCommission.updateMany({ where: { payoutId: id }, data: { payoutId: null, status: "APPROVED" } });
            return payout;
        });
    }
};
