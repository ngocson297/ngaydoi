...
const albumToken = () => randomBytes(32).toString("base64url");
...
function fileSignatureMatches(mimeType, buffer) {
...
let MemoriesService = class MemoriesService {
    ...
    storage;
    rateLimit;
    constructor(prisma, storage, rateLimit) {
        this.prisma = prisma;
    ...
    async access(weddingId, user, edit = false) {
        ...
        if (!permission)
            throw new NotFoundException("Không tìm thấy đám cưới");
        if (edit && permission === "VIEW")
            throw new ForbiddenException("Bạn chỉ có quyền xem album");
        return { ownerId: wedding.ownerId, access: permission };
    }
    async ensureAlbum(weddingId) {
    ...
    async assertCapacity(albumId, incomingBytes) {
        const limits = this.uploadLimits();
        const usage = await this.albumUsage(albumId);
        if (usage.count >= limits.maxItems)
            throw new ForbiddenException("Album đã đạt giới hạn số lượng nội dung.");
        if (usage.bytes + incomingBytes > limits.maxBytes)
            throw new ForbiddenException("Album đã đạt giới hạn dung lượng.");
    }
    validateUploadMetadata(mimeType, sizeBytes, direct) {
        if (!ALLOWED_MIME.has(mimeType))
            throw new BadRequestException("Chỉ hỗ trợ JPEG, PNG, WebP, MP4, WebM và MOV");
        if (!Number.isFinite(sizeBytes) || sizeBytes < 1)
            throw new BadRequestException("Dung lượng file không hợp lệ");
        const limits = this.uploadLimits();
    ...
    actorHash(token, actorKey) {
        const key = cleanText(actorKey, 160);
        if (key.length < 12)
            throw new BadRequestException("Phiên tương tác không hợp lệ");
    ...
    verifyUploadTicket(value) {
        ...
        if (payload.expiresAt <= Date.now())
            throw new BadRequestException("Upload ticket đã hết hạn");
        return payload;
    }
    async resolveInvitation(albumWeddingId, invitationToken) {
        ...
        if (!invitation || invitation.guest.weddingId !== albumWeddingId)
            throw new BadRequestException("Liên kết khách mời không thuộc album này");
        return { id: invitation.id, guestName: invitation.guest.fullName };
    }
    assetMediaUrl(asset, token) {
        return /^https?:\/\//i.test(asset.publicUrl) ? asset.publicUrl : `/memories/assets/${asset.id}?token=${encodeURIComponent(token)}`;
    }
    async decoratePublicAssets(token, assets, viewerKey) {
        if (!assets.length)
            return [];
    ...
    async publicAssetPage(albumId, token, cursor, limit, viewerKey) {
    ...
    async ownerOverview(weddingId, user) {
        ...
        if (!album)
            throw new NotFoundException("Không tìm thấy album");
    ...
    async ownerAssetPage(albumId, token, cursor, limit, status) {
        const where = { albumId };
    ...
    async ownerAssets(weddingId, user, cursor, limitValue, status = "ALL") {
    ...
    async publicOverview(token, viewerKey) {
        const album = await this.prisma.memoryAlbum.findUnique({
            where: { token },
            include: { wedding: { select: { title: true, brideName: true, groomName: true, mainDate: true, coverImageUrl: true } } },
        });
        if (!album || !album.publicEnabled)
            throw new NotFoundException("Album chưa được mở công khai");
    ...
    async updateSettings(weddingId, body, user) {
        ...
        if (closesAt instanceof Date && Number.isNaN(closesAt.getTime()))
            throw new BadRequestException("Thời hạn upload không hợp lệ");
    ...
    async createAssetFromMetadata(album, metadata, publicUrl) {
        ...
        if (resolvedPublicUrl !== asset.publicUrl)
            await this.prisma.memoryAsset.update({ where: { id: asset.id }, data: { publicUrl: resolvedPublicUrl } });
    ...
    async prepareUpload(token, body) {
        const album = await this.prisma.memoryAlbum.findUnique({ where: { token }, include: { wedding: { select: { id: true, ownerId: true } } } });
        if (!album || !album.publicEnabled)
            throw new NotFoundException("Album chưa được mở công khai");
        if (!album.uploadEnabled || (album.closesAt && album.closesAt <= new Date()))
            throw new ForbiddenException("Album đang tạm ngừng nhận nội dung mới");
    ...
    async completeUpload(token, body) {
        ...
        if (!album || album.id !== metadata.albumId || album.weddingId !== metadata.weddingId || !album.publicEnabled)
            throw new NotFoundException("Album chưa được mở công khai");
        await this.assertCapacity(album.id, metadata.sizeBytes);
        let head;
        try {
            head = await this.storage.head(metadata.storageKey);
        }
        catch {
            throw new BadRequestException("File chưa được upload hoàn tất lên storage");
        }
    ...
    async upload(token, file, body) {
        if (!file)
            throw new BadRequestException("Vui lòng chọn ảnh hoặc video");
        const album = await this.prisma.memoryAlbum.findUnique({ where: { token }, include: { wedding: { select: { id: true, ownerId: true } } } });
        if (!album || !album.publicEnabled)
            throw new NotFoundException("Album chưa được mở công khai");
        if (!album.uploadEnabled)
            throw new ForbiddenException("Album đang tạm ngừng nhận nội dung mới");
        if (album.closesAt && album.closesAt <= new Date())
            throw new ForbiddenException("Thời gian nhận ảnh đã kết thúc");
        const { extension } = this.validateUploadMetadata(file.mimetype, file.size, false);
        if (!fileSignatureMatches(file.mimetype, file.buffer))
            throw new BadRequestException("Nội dung file không khớp với định dạng đã chọn");
    ...
    parseArchiveAssetIds(value) {
        ...
        if (!ids.length)
            return undefined;
        if (ids.length > 200)
            throw new BadRequestException("Mỗi lượt chỉ có thể tải tối đa 200 nội dung đã chọn.");
        return ids;
    }
    async approvedArchiveAssets(albumId, assetIds) {
        const rows = await this.prisma.memoryAsset.findMany({
        ...
        if (!rows.length)
            throw new BadRequestException("Không có ảnh hoặc video đã duyệt để tải.");
    ...
    async ownerArchive(weddingId, user, assetIdsValue) {
        await this.access(weddingId, user);
    ...
    async toggleReaction(token, assetId, body) {
        const album = await this.prisma.memoryAlbum.findUnique({ where: { token }, select: { id: true, publicEnabled: true, reactionsEnabled: true } });
        if (!album?.publicEnabled)
            throw new NotFoundException("Album chưa được mở công khai");
        if (!album.reactionsEnabled)
            throw new ForbiddenException("Album đang tắt tương tác cảm xúc");
        ...
        if (existing)
            ...
            await this.prisma.memoryReaction.create({ data: { assetId, actorHash: hash, type: "HEART" } });
        const count = await this.prisma.memoryReaction.count({ where: { assetId, type: "HEART" } });
        return { reacted: !existing, count };
    }
    async comments(token, assetId, cursor, limitValue, viewerKey) {
        ...
        if (cleanText(viewerKey, 160)) {
            try {
                viewerHash = this.actorHash(token, viewerKey);
            }
            catch {
                viewerHash = null;
            }
    ...
    async addComment(token, assetId, body) {
        ...
        if (!album?.publicEnabled)
            throw new NotFoundException("Album chưa được mở công khai");
        if (!album.commentsEnabled)
            throw new ForbiddenException("Album đang tắt bình luận");
        const asset = await this.prisma.memoryAsset.findFirst({ where: { id: assetId, albumId: album.id, status: "APPROVED" }, select: { id: true } });
        if (!asset)
            throw new NotFoundException("Không tìm thấy khoảnh khắc");
        const hash = this.actorHash(token, body.actorKey);
    ...
    async deleteOwnComment(token, assetId, commentId, body) {
        const album = await this.prisma.memoryAlbum.findUnique({ where: { token }, select: { id: true, publicEnabled: true, commentsEnabled: true } });
        if (!album?.publicEnabled)
            throw new NotFoundException("Album chưa được mở công khai");
        if (!album.commentsEnabled)
    ...
    async publicGuestbookPage(weddingId, cursor, limit) {
    ...
    async guestbook(token, cursor, limitValue) {
    ...
    async moderateSocial(weddingId, kind, id, body, user) {
        await this.access(weddingId, user, true);
        const status = cleanText(body.status, 20).toUpperCase();
        if (!new Set(["APPROVED", "HIDDEN"]).has(status))
            throw new BadRequestException("Trạng thái kiểm duyệt không hợp lệ");
        const now = new Date();
        if (kind === "comment") {
            const comment = await this.prisma.memoryComment.findFirst({ where: { id, asset: { album: { weddingId } } } });
            if (!comment)
                throw new NotFoundException("Không tìm thấy bình luận");
            return this.prisma.memoryComment.update({ where: { id }, data: { status: status, approvedAt: status === "APPROVED" ? now : null, hiddenAt: status === "HIDDEN" ? now : null } });
        }
        if (kind === "guestbook") {
            ...
            return this.prisma.guestbookEntry.update({ where: { id }, data: { status: status, approvedAt: status === "APPROVED" ? now : null, hiddenAt: status === "HIDDEN" ? now : null } });
        }
        throw new BadRequestException("Loại nội dung không hợp lệ");
    }
    async deleteOwnerComment(weddingId, commentId, user) {
        await this.access(weddingId, user, true);
    ...
    async moderate(weddingId, assetId, body, user) {
        await this.access(weddingId, user, true);
        ...
        if (!new Set(["APPROVED", "REJECTED", "ARCHIVED"]).has(status))
            throw new BadRequestException("Trạng thái kiểm duyệt không hợp lệ");
        const now = new Date();
    ...
    async setFeatured(weddingId, assetId, body, user) {
        await this.access(weddingId, user, true);
        const album = await this.prisma.memoryAlbum.findUnique({ where: { weddingId }, select: { id: true } });
        if (!album)
            throw new NotFoundException("Không tìm thấy album");
        ...
        if (asset.status !== "APPROVED")
            throw new BadRequestException("Chỉ ảnh/video đã duyệt mới có thể đặt làm khoảnh khắc nổi bật");
        if (asset.featuredOrder !== null)
            return asset;
        ...
        if (count >= 12)
            throw new BadRequestException("Trang kỷ niệm chỉ hỗ trợ tối đa 12 khoảnh khắc nổi bật");
        return this.prisma.memoryAsset.update({
            where: { id: assetId },
            data: { featuredOrder: (maxOrder._max.featuredOrder ?? 0) + 1, featuredAt: new Date() },
        });
    }
    async bulkModerate(weddingId, body, user) {
        await this.access(weddingId, user, true);
        const ids = Array.isArray(body.assetIds) ? [...new Set(body.assetIds.map((value) => cleanText(value, 80)).filter(Boolean))] : [];
        const status = cleanText(body.status, 20);
        if (!ids.length)
            throw new BadRequestException("Vui lòng chọn ít nhất một nội dung");
        if (!new Set(["APPROVED", "REJECTED", "ARCHIVED"]).has(status))
            throw new BadRequestException("Trạng thái kiểm duyệt không hợp lệ");
        const album = await this.prisma.memoryAlbum.findUnique({ where: { weddingId }, select: { id: true } });
        if (!album)
        ...
        const result = await this.prisma.memoryAsset.updateMany({
            ...
            data: { status: status, rejectionReason: status === "REJECTED" ? cleanText(body.rejectionReason, 300) || "Không phù hợp với album" : null, featuredOrder: status === "APPROVED" ? undefined : null, featuredAt: status === "APPROVED" ? undefined : null, approvedAt: status === "APPROVED" ? now : null, rejectedAt: status === "REJECTED" ? now : null },
        });
        return { updated: result.count };
    }
    async remove(weddingId, assetId, user) {
        await this.access(weddingId, user, true);
        const asset = await this.prisma.memoryAsset.findFirst({ where: { id: assetId, album: { weddingId } } });
        if (!asset)
            throw new NotFoundException("Không tìm thấy nội dung");
    ...
    async media(assetId, token, download = false) {
        ...
        if (!asset || asset.album.token !== token || !asset.album.publicEnabled || !["APPROVED", "PENDING"].includes(asset.status))
            throw new NotFoundException("Không tìm thấy nội dung");
        if (download && !asset.album.downloadsEnabled)
            throw new ForbiddenException("Album đang tắt tải file");
        try {
    ...
    async ownerMedia(weddingId, assetId, user) {
        await this.access(weddingId, user);
        const asset = await this.prisma.memoryAsset.findFirst({ where: { id: assetId, album: { weddingId } } });
        if (!asset)
            throw new NotFoundException("Không tìm thấy nội dung");
