...
let WeddingsService = class WeddingsService {
    ...
    invitationService;
    entitlements;
    constructor(prisma, invitationService, entitlements) {
        this.prisma = prisma;
    ...
    ensureSlugAllowed(slug) {
        if (RESERVED_SLUGS.has(slug))
            throw new ConflictException("This invitation URL is reserved");
    }
    async assertSlugAvailable(slug, excludeWeddingId) {
        ...
        if (existing)
            throw new ConflictException("This invitation URL is already in use");
    }
    ...
    requireEdit(access) {
        if (access === "VIEW")
            throw new ForbiddenException("You only have view access to this wedding");
    }
    requireOwner(access) {
        if (access !== "OWNER")
            throw new ForbiddenException("Only the wedding owner can perform this action");
    }
    buildChecklist(wedding) {
        const coupleComplete = Boolean(wedding.brideName.trim() && wedding.groomName.trim());
        const familyComplete = Boolean(wedding.brideFatherName || wedding.brideMotherName || wedding.groomFatherName || wedding.groomMotherName);
    ...
    async getSlugAvailability(rawSlug, excludeWeddingId) {
        ...
        if (RESERVED_SLUGS.has(slug))
            return { slug, available: false, reason: "This URL is reserved" };
        const existing = await this.prisma.wedding.findFirst({
            where: { slug, ...(excludeWeddingId ? { id: { not: excludeWeddingId } } : {}) },
            select: { id: true },
        });
    ...
    async getPublicBySlug(slug) {
        ...
        const wedding = await this.prisma.wedding.findFirst({
            where: {
                ...
                status: "PUBLISHED",
                OR: [{ expiresAt: null }, { expiresAt: { gt: now } }, { memoryAlbum: { is: { memoryModeEnabled: true } } }],
            },
            select: {
                id: true,
                slug: true,
        ...
        if (!wedding)
            throw new NotFoundException("Wedding invitation not found");
        return wedding;
    }
    ...
    async updateLifecycle(id, dto, user) {
        ...
        if (dto.status === "PUBLISHED") {
            ...
            if (access.plan.code === "FREE")
                throw new BadRequestException("Vui lòng chọn gói dịch vụ trước khi xuất bản thiệp.");
    ...
    async requestPublish(id, user) {
        ...
        if (!wedding.mainDate || eventCount < 1)
            throw new BadRequestException("Vui lòng hoàn tất ngày cưới và ít nhất một sự kiện trước khi gửi duyệt.");
        if (entitlement.plan.code === "FREE")
            throw new BadRequestException("Vui lòng chọn gói dịch vụ trước khi gửi yêu cầu xuất bản.");
        const now = new Date();
    ...
    async createEvent(weddingId, dto, user) {
    ...
    async updateEvent(weddingId, eventId, dto, user) {
    ...
    async deleteEvent(weddingId, eventId, user) {
        const { access } = await this.getAccess(weddingId, user);
        this.requireEdit(access);
    ...
    async inviteCollaborator(weddingId, dto, user) {
        const { access } = await this.getAccess(weddingId, user);
        this.requireOwner(access);
        ...
        if (existing?.status === "ACCEPTED")
            throw new ConflictException("This collaborator already has access");
    ...
    async acceptCollaboration(token, user) {
    ...
    async revokeCollaborator(weddingId, collaboratorId, user) {
        const { access } = await this.getAccess(weddingId, user);
        this.requireOwner(access);
