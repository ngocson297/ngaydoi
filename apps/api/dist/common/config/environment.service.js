...
let EnvironmentService = EnvironmentService_1 = class EnvironmentService {
    logger = new Logger(EnvironmentService_1.name);
    onApplicationBootstrap() {
        ...
        if (errors.length)
            throw new Error(`Unsafe production configuration: ${errors.map((item) => item.key).join(", ")}`);
    }
    report() {
        const production = process.env.NODE_ENV === "production";
        const checks = [];
        const status = (ready) => ready ? "ok" : production ? "error" : "warning";
