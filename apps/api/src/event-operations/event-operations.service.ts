...
const textValue = (value: unknown, max = 160): string => String(value ?? "").trim().slice(0, max);
...
export class EventOperationsService {
  ...
  private async access(weddingId: string, user: AuthenticatedUser, edit = false): Promise<{ wedding: { id: string; title: string; ownerId: string }; access: "OWNER" | "EDIT" | "VIEW" }> {
    ...
    if (wedding.ownerId !== user.id && !wedding.collaborators[0]) throw new NotFoundException("Không tìm thấy đám cưới");
    if (edit && access === "VIEW") throw new ForbiddenException("Bạn chỉ có quyền xem");
    return { wedding: { id: wedding.id, title: wedding.title, ownerId: wedding.ownerId }, access };
  }

  private async scope(weddingId: string, eventId?: string | null): Promise<{ eventId: string | null; eventKey: string }> {
    const clean = textValue(eventId, 80);
    if (!clean) return { eventId: null, eventKey: "WEDDING" };
    const event = await this.prisma.event.findFirst({ where: { id: clean, weddingId }, select: { id: true } });
    if (!event) throw new BadRequestException("Sự kiện không thuộc đám cưới này");
    return { eventId: event.id, eventKey: `EVENT:${event.id}` };
  }

  async overview(weddingId: string, eventId: string | undefined, user: AuthenticatedUser): Promise<unknown> {
  ...
  async createTable(weddingId: string, body: Record<string, unknown>, user: AuthenticatedUser): Promise<unknown> {
    await this.access(weddingId, user, true);
    const scope = await this.scope(weddingId, body.eventId as string | undefined);
    const name = textValue(body.name, 80);
    const code = textValue(body.code || name, 30).toUpperCase().replace(/[^A-Z0-9_-]/g, "-");
    if (!name || !code) throw new BadRequestException("Vui lòng nhập tên và mã bàn");
    const capacity = Math.min(50, Math.max(1, numberValue(body.capacity, 10)));
    try {
      return await this.prisma.seatingTable.create({ data: { weddingId, ...scope, name, code, capacity, zone: textValue(body.zone, 80) || null, shape: (["ROUND", "RECTANGLE", "LONG", "OTHER"].includes(String(body.shape)) ? String(body.shape) : "ROUND") as never, note: textValue(body.note, 300) || null, sortOrder: numberValue(body.sortOrder, 0) } });
    } catch { throw new ConflictException("Mã bàn đã được sử dụng trong sự kiện này"); }
  }

  async updateTable(weddingId: string, tableId: string, body: Record<string, unknown>, user: AuthenticatedUser): Promise<unknown> {
    await this.access(weddingId, user, true);
    ...
    if (capacity !== undefined && table.assignments.reduce((sum, item) => sum + item.seatCount, 0) > capacity) throw new BadRequestException("Sức chứa mới nhỏ hơn số ghế đã phân");
    return this.prisma.seatingTable.update({ where: { id: tableId }, data: { name: body.name === undefined ? undefined : textValue(body.name, 80), capacity, zone: body.zone === undefined ? undefined : textValue(body.zone, 80) || null, shape: body.shape === undefined ? undefined : String(body.shape) as never, note: body.note === undefined ? undefined : textValue(body.note, 300) || null, sortOrder: body.sortOrder === undefined ? undefined : numberValue(body.sortOrder) } });
  }

  async deleteTable(weddingId: string, tableId: string, user: AuthenticatedUser): Promise<unknown> {
    await this.access(weddingId, user, true);
    const table = await this.prisma.seatingTable.findFirst({ where: { id: tableId, weddingId }, include: { _count: { select: { assignments: true } } } });
    if (!table) throw new NotFoundException("Không tìm thấy bàn");
    if (table._count.assignments) throw new BadRequestException("Hãy bỏ phân bàn cho khách trước khi xóa bàn");
    await this.prisma.seatingTable.delete({ where: { id: tableId } });
    return { deleted: true };
  }

  async assignGuest(weddingId: string, body: Record<string, unknown>, user: AuthenticatedUser): Promise<unknown> {
    await this.access(weddingId, user, true);
    ...
    if (!guest || !table) throw new NotFoundException("Không tìm thấy khách hoặc bàn");
    ...
    if (occupied + seatCount > table.capacity) throw new BadRequestException("Bàn không còn đủ chỗ");
  ...
  async unassignGuest(weddingId: string, assignmentId: string, user: AuthenticatedUser): Promise<unknown> {
    await this.access(weddingId, user, true);
    const result = await this.prisma.seatAssignment.deleteMany({ where: { id: assignmentId, weddingId } });
    if (!result.count) throw new NotFoundException("Không tìm thấy phân bàn");
    return { removed: true };
  }

  async autoAssign(weddingId: string, body: Record<string, unknown>, user: AuthenticatedUser): Promise<unknown> {
    await this.access(weddingId, user, true);
    const scope = await this.scope(weddingId, body.eventId as string | undefined);
    ...
    if (!tables.length) throw new BadRequestException("Hãy tạo ít nhất một bàn trước");
    let assigned = 0;
  ...
  async createStation(weddingId: string, body: Record<string, unknown>, user: AuthenticatedUser): Promise<unknown> {
    await this.access(weddingId, user, true);
    const scope = await this.scope(weddingId, body.eventId as string | undefined);
  ...
  async updateStation(weddingId: string, stationId: string, body: Record<string, unknown>, user: AuthenticatedUser): Promise<unknown> {
    await this.access(weddingId, user, true);
    const station = await this.prisma.checkinStation.findFirst({ where: { id: stationId, weddingId } });
    if (!station) throw new NotFoundException("Không tìm thấy trạm check-in");
    return this.prisma.checkinStation.update({ where: { id: stationId }, data: { name: body.name === undefined ? undefined : textValue(body.name, 80), status: body.status === undefined ? undefined : String(body.status) as never, token: body.regenerateToken ? randomBytes(24).toString("base64url") : undefined } });
  }

  private async activeStation(token: string): Promise<{ id: string; weddingId: string; eventId: string | null; eventKey: string; name: string }> {
    ...
    if (!station || station.status !== "ACTIVE") throw new NotFoundException("Link check-in không còn hoạt động");
    await this.prisma.checkinStation.update({ where: { id: station.id }, data: { lastUsedAt: new Date() } });
    return station;
  }

  async stationOverview(token: string): Promise<unknown> {
  ...
  async searchStationGuests(token: string, query: string): Promise<unknown> {
    const station = await this.activeStation(token);
  ...
  async checkOut(token: string, body: Record<string, unknown>): Promise<unknown> {
    ...
    if (!record || record.weddingId !== station.weddingId) throw new NotFoundException("Không tìm thấy lượt check-in");
    return this.prisma.checkinRecord.update({ where: { id: record.id }, data: { checkedOutAt: new Date() } });
  }

  async guestQr(invitationToken: string): Promise<string> {
  ...
  async exportCsv(weddingId: string, eventId: string | undefined, user: AuthenticatedUser): Promise<string> {
    ...
    for (const guest of guests) {
      const assignment = guest.seatAssignments[0]; const checkin = guest.checkinRecords[0]; const rsvp = guest.invitations[0]?.rsvp;
