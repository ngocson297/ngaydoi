...
import { PlanCode, type Prisma } from "../generated/prisma/client.js";
...
export class CommercialService {
  ...
  private toPlanSummary(plan: Prisma.PlanGetPayload<{ select: typeof planSelect }>): PlanSummary {
    return { ...plan, recommended: plan.code === "STANDARD" };
  }

  private async assertWeddingOwner(weddingId: string, userId: string) {
  ...
  async getEntitlements(weddingId: string, user: AuthenticatedUser) {
    await this.assertWeddingOwner(weddingId, user.id);
    return this.entitlements.getWeddingEntitlements(weddingId);
  }

  private async resolveQuote(dto: CreateOrderDto): Promise<OrderQuote & { planId: string; couponId: string | null; addOnIds: string[] }> {
    ...
    if (!planCode) throw new BadRequestException("Mã gói dịch vụ không hợp lệ");
    const plan = await this.prisma.plan.findFirst({ where: { code: planCode, active: true }, select: planSelect });
    if (!plan) throw new NotFoundException("Plan not found");
    if (plan.code === "FREE") throw new BadRequestException("Gói miễn phí không cần tạo đơn hàng");
    ...
    const addOns = requestedCodes.length
      ...
      : [];
    if (addOns.length !== requestedCodes.length) throw new BadRequestException("Một hoặc nhiều add-on không hợp lệ");

    const subtotalAmount = plan.priceAmount + addOns.reduce((sum, item) => sum + item.priceAmount, 0);
    let coupon: { id: string; code: string; name: string; discountType: string; discountValue: number; usageLimit: number | null; usedCount: number; planCodes: string[] } | null = null;
    ...
    if (dto.couponCode) {
      ...
      if (!coupon) throw new BadRequestException("Mã ưu đãi không tồn tại hoặc đã hết hạn");
      if (coupon.usageLimit !== null && coupon.usedCount >= coupon.usageLimit) throw new BadRequestException("Mã ưu đãi đã hết lượt sử dụng");
      if (coupon.planCodes.length && !coupon.planCodes.includes(plan.code)) throw new BadRequestException("Mã ưu đãi không áp dụng cho gói này");
  ...
  async quote(dto: CreateOrderDto, user: AuthenticatedUser): Promise<OrderQuote> {
  ...
  async submitPayment(orderId: string, dto: SubmitPaymentDto, user: AuthenticatedUser) {
    const order = await this.getOrder(orderId, user);
    if (["CONFIRMED", "REFUNDED"].includes(order.paymentStatus)) throw new BadRequestException("Đơn hàng đã được xử lý thanh toán");
  ...
  async sandboxPay(orderId: string, user: AuthenticatedUser) {
    if (process.env.PAYMENT_SANDBOX_ENABLED !== "true") throw new ForbiddenException("Sandbox payment is disabled");
    const order = await this.getOrder(orderId, user);
    const payment = order.payments[0];
    if (!payment) throw new NotFoundException("Payment not found");
    if (payment.status === "CONFIRMED") return this.getOrder(orderId, user);
    await this.activateOrder(orderId, payment.id, user.id, "SANDBOX");
    return this.getOrder(orderId, user);
  }

  async activateOrder(orderId: string, paymentId: string, reviewerId: string, provider = "MANUAL") {
    const current = await this.prisma.order.findUnique({ where: { id: orderId }, include: { plan: true, coupon: true } });
    if (!current) throw new NotFoundException("Order not found");
    if (current.paymentStatus === "REFUNDED" || current.status === "REFUNDED") throw new BadRequestException("Đơn đã hoàn tiền không thể kích hoạt lại");
    if (current.paymentStatus === "CONFIRMED" && current.fulfillmentStatus === "ACTIVE") return current;
    const now = new Date();
    const activated = await this.prisma.$transaction(async (tx) => {
      ...
      if (claimed.count === 0) return tx.order.findUniqueOrThrow({ where: { id: orderId } });
      ...
      if (current.couponId) await tx.coupon.update({ where: { id: current.couponId }, data: { usedCount: { increment: 1 } } });
