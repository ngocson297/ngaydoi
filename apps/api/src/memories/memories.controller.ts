...
export class MemoriesController {
  ...
  publicOverview(@Param("token") token: string): Promise<unknown> {
    return this.memories.publicOverview(token);
  }
  ...
  media(@Param("assetId") assetId: string, @Query("token") token: string): Promise<unknown> {
    return this.memories.media(assetId, token);
  }
